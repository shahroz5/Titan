...
export class CoToleranceConfigAdaptor {
  static getCoToleranceConfigListData(
    data: any
  ): { data: CoToleranceConfigResponse[]; totalElements: number } {
    const residualWeightConfiguration: CoToleranceConfigResponse[] = [];
    if (data && data.results) {
      for (const listData of data.results) {
        residualWeightConfiguration.push({
          ...
          lastModifiedDate: listData.lastModifiedDate
            ? moment(listData.lastModifiedDate)
            : ''
        });
      }
    }
  ...
  static getSelectedConfigData(data: any): CoToleranceConfigResponse {
    let configData;
    configData = {
      ruleId: data.ruleId,
  ...
  static getCoToleranceConfiguration(data: any): CoToleranceConfigResponse {
    let config: CoToleranceConfigResponse;
    if (data) {
      config = {
  ...
  static getCoToleranceRangeWeight(data: any): CoToleranceWeightRange[] {
  ...
  // static getWeightValueConfigDetailsNew(): CoToleranceConfigResponse {
  //   const weightValueConfigDetails: CoToleranceConfigResponse = {
  //     isActive: true,
  //     description: '',
  //     ruleId: null,
  ...
  // }
  static getConfigMapping(data: any): CoToleranceConfigDetailsResPayload {
    const range: any = [];
  ...
  static getCoToleranceConfigMetalTypes(
    data: any
  ): CoToleranceConfigMetalType[] {
    const metalTypes: CoToleranceConfigMetalType[] = [];
