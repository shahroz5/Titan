...
@Component({
  selector: 'poss-web-customer-config-form',
  template: `
...
export class CustomerConfigFormComponent implements OnInit, OnDestroy {
  constructor(
  ...
  prepareSet() {
    ...
    const customerConfigModel = new CustomerConfigModel(
      1,
      this.brandMasterDetails?.customerDetails?.data?.regularMobileNoStartsWith?.join() ||
        '',
      this.brandMasterDetails?.customerDetails?.data?.oneTimeMobileNoStartsWith?.join() ||
        '',
      this.brandMasterDetails?.customerDetails?.data?.institutionalMobileNoStartsWith?.join() ||
        '',
      this.brandMasterDetails?.customerDetails?.data?.internationalMobileNoStartsWith?.join() ||
        '',
      this.fieldValidatorsService,
      this.translateService
    );

    const detailsmain = new CustomerConfigFormModel(1, customerConfigModel);
  ...
  public setFormConfig() {
    return {
      formName: 'pw.brandMaster.customerConfigurations',
      formDesc: '',
  ...
  public addButton(formGroup: FormGroup) {
    ...
    const brandDetails: BrandMasterDetails = {
      ...
      customerDetails: {
        ...
        data: {
          institutionalMobileNoStartsWith: formData['1-customerConfigModel'][
            '1-institutionalCustomerMNSS'
          ].split(','),
          internationalMobileNoStartsWith: formData['1-customerConfigModel'][
            '1-internationalCustomerMNSS'
          ].split(','),
          oneTimeMobileNoStartsWith: formData['1-customerConfigModel'][
            '1-oneTimeCustomerMNSS'
          ].split(','),
          regularMobileNoStartsWith: formData['1-customerConfigModel'][
            '1-encircleCustomerMNSS'
          ].split(',')
        }
      }
    };
