...
@Component({
  selector: 'poss-web-customer-gst-no-search',
  templateUrl: './customer-gst-no-search.component.html',
  styleUrls: ['./customer-gst-no-search.component.scss']
})
export class CustomerGstNoSearchComponent
  ...
  validKeyCheck($event: KeyboardEvent) {
    ...
    if (isValid) {
      ...
      const validLength = fieldValidation.gstNumberField.maxLength;
