...
} from './location-settings.reducer';
import { CustomErrorAdaptor } from '@poss-web/shared/util-adaptors';
import * as actions from './location-settings.actions';
import { LocationSettingDetails } from '@poss-web/shared/models';

describe('Location Settings reducer Testing Suite', () => {
  describe('Testing Load Save Walk In Details Functionality', () => {
    beforeEach(() => {});
    it('Testing LOAD_LOCATION_SETTINGS', () => {
      const requestPayload = {
        isBTQUser: true,
        countryName: 'IND'
      };
      const action = new actions.LoadLocationSettings(requestPayload);
      const result: LocationSettingsState = LocationSettingsReducer(
        initialState,
        action
      );
      expect(result.locationSettingsData).toBe(null);
      expect(result.error).toBe(null);
    });
    it('LOAD_LOCATION_SETTINGS_SUCCESS should update locationSettingsData field in state', () => {
      ...
      const action = new actions.LoadLocationSettingsSuccess(
      ...
      expect(result.locationSettingsData).toBe(locationSettingDetails);
    });
    it('LOAD_LOCATION_SETTINGS_FAILURE should return error', () => {
      const action = new actions.LoadLocationSettingsFailure(
