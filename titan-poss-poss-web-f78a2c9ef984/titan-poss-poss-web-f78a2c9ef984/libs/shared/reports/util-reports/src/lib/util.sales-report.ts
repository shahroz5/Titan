...
export const generateSalesReport = (
  ...
  if (form.valid) {
    ...
    const request: GenerateReportRequest = {
      fromDate: (form.get('fromDate').value as Moment).startOf('day').valueOf(),
      toDate: (form.get('toDate').value as Moment).endOf('day').valueOf(),
      ...
      customFields: {
        data: {
          ...
          docNo: formValue.docNo ? formValue.docNo : null,
          ...
          complexity: formValue.complexity ? formValue.complexity : null,
          karatage: formValue.karatage ? formValue.karatage : []
        },
        type: 'string'
      }
    };
