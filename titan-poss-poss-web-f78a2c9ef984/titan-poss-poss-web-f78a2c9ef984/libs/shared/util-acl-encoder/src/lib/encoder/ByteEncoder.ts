...
export default class ByteEncoder {
   ...
   * @return Hex string
   */
  static hexStringFromBytes(bytes: Uint8Array) {
    return Array.from(bytes)
      .map(byte => (zero + byte.toString(16)).slice(-2))
      .join('');
  }
  ...
  static bytesFromHexString(string) {
    ...
    const bytes = StringUtil.chunk(string, 2).map((byteString, index) => {
      ...
      if (byteString.match(/[0-9a-f]{2}/i) === null || isNaN(byte)) {
        throw new ByteEncodingError(`Invalid hex encoded byte '${byteString}'`);
      }
      return byte;
    });
   ...
   */
  static binaryStringFromBytes(bytes: Uint8Array) {
    return Array.from(bytes)
      .map(byte => (zeroByte + byte.toString(2)).slice(-8))
      .join('');
  }
   ...
   * @param [options.alphabet] Base64 alphabet
   * @param [options.padding='='] Padding character
   * @param [options.paddingOptional=false] Wether padding is optional
   * @param [options.foreignCharacters=false] Wether foreign
   * @param [options.maxLineLength=null] Maximum line length
   * @param [options.lineSeparator='\r\n'] Line separator, if
   * having a maximum line length.
   * @return Base64 string
   */
  static base64StringFromBytes(bytes: Uint8Array, options = {}) {
    // Compose options
    const {
      alphabet,
      padding,
   ...
   * @param  [options.lineSeparator='\r\n'] Line separator, if
   * having a maximum line length.
   * @throws If the string contains forbidden characters.
   * @throws If the string unexpectedly ends.
   * @return Base64 string
   */
  // @dynamic
