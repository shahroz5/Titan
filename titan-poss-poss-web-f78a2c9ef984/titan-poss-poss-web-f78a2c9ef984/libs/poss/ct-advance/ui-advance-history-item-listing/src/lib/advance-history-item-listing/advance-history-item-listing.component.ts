...
@Component({
  selector: 'poss-web-advance-history-item-listing',
  templateUrl: './advance-history-item-listing.component.html'
  // changeDetection: ChangeDetectionStrategy.OnPush
})
export class AdvanceHistoryItemListingComponent
  ...
  @Input() advanceHistoryItemList: AdvanceHistoryItem[];
  @Input() itemTotalCount: any;
  @Input() advanceHistoryObservable: Observable<any>;
  ...
  ngOnInit() {
    ...
    const paymentMadeForEghsLabel = 'pw.acceptAdvance.paymentMadeForEghs';
    const createdByLabel = 'pw.acceptAdvance.createdBy';
    const docDateLabel = 'pw.acceptAdvance.documentDate';
    ...
    this.translate
      ...
      .subscribe((translatedMessages: string) => {
        this.columnDefs = [
          ...
          {
            headerName: translatedMessages[paymentMadeForEghsLabel],
            field: 'eghsPayment',
          ...
          {
            headerName: translatedMessages[docDateLabel],
            field: 'docDate',
  ...
  ngOnChanges() {
    ...
    this.count = this.itemTotalCount;
    this.rowData = [];
    if (this.advanceHistoryItemList) {
      const rowData = [...this.advanceHistoryItemList];
      this.rowData = rowData.map(rowItem => {
        let rowDetail: any;
        rowDetail =
          rowItem &&
          rowItem.eghsPayment &&
          typeof rowItem.eghsPayment === 'string'
            ? JSON.parse(rowItem.eghsPayment)
            : rowItem.eghsPayment;
        const eghsPayment =
          rowDetail && rowDetail.data && rowDetail.data.isPaymentForEGHS
            ? rowDetail.data.isPaymentForEGHS
            : null;
        return {
          ...rowItem,
          formGroup: new FormGroup({
  ...
  onCellFocused(event) {
    ...
    if (this.currentRowField === 'eghsPayment')
      this.currentRowInfo = JSON.parse(
        node.data.eghsPayment
      ).data.isPaymentForEGHS;
  }
