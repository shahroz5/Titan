...
@Component({
  selector: 'poss-web-discount-bill-level',
  templateUrl: './discount-bill-level.component.html',
  styleUrls: ['./discount-bill-level.component.scss']
})
export class DiscountBillLevelComponent
  implements OnInit, OnChanges, OnDestroy {
  billLevelDiscount: FormGroup;
  ...
  constructor(
    ...
    this.billLevelDiscount = this.fb.group({
  ...
  createFormGroup(discount: any) {
    return new FormGroup({
      ...
      selectedDiscountValue: new FormControl(null, [
        // (this.fieldValidatorService.requiredField('Bill Discount Value'),
  ...
  updateAppliedDiscounts() {
    for (const appliedDiscount of this.appliedBillLevelDiscounts) {
      ...
      for (const formData of this.discountsFormArray?.controls) {
        // for (const formData of this.billLevelDiscount?.controls) {
        if (appliedDiscount.discountId == formData.value.discountId) {
          const controlArray = <FormArray>(
            this.billLevelDiscount.controls['discountsFormArray']
          );
  ...
  //   return this.billLevelDiscount.get('discountsFormArray')[
  //     'controls'
  //   ] as FormArray;
  // }
  get discountsFormArray(): FormArray {
    return this.billLevelDiscount.get('discountsFormArray') as FormArray;
  }
      ...
      if (this.selectedDiscountIds.length === 0) {
        ...
        this.errorMsg = !selectedDiscount.isOtherBillLevelDiscount
          ? selectedDiscount?.discountCode +
            ' cannot be clubbed with other Bill Level discounts'
  ...
  onApply() {
    this.billLevelDiscountPayload = [];
    this.selectedDiscounts = this.billLevelDiscount
      .get('discountsFormArray')
      .value.filter(discount => discount.isSelected === true);

    for (const discount of this.selectedDiscounts) {
      this.billLevelDiscountPayload.push(this.createPayload(discount));
    }
    this.applyDiscount.emit(this.billLevelDiscountPayload);
  }
  createPayload(discount) {
    return {
      ...
      discountValue:
        discount.isBillValue === true && discount.selectedDiscountValue !== ''
  ...
  clear() {
    this.billLevelDiscount.setControl('discountsFormArray', this.fb.array([]));
  }
  ngOnDestroy() {
    this.billLevelDiscount = this.fb.group({
      discountsFormArray: this.fb.array([])
