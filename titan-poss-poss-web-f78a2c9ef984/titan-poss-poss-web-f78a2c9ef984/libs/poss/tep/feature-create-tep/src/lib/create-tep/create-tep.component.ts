...
import { getCreateTepUrl } from '@poss-web/shared/util-site-routes';
...
@Component({
  selector: 'poss-web-create-tep',
  templateUrl: './create-tep.component.html',
  styleUrls: ['./create-tep.component.scss']
})
export class CreateTepComponent implements OnInit, OnChanges, OnDestroy {
  ...
  customerConfigAlertMessage: string;
  remarks = '';
  isTepRequestApprovalWorkflow = false;
  ...
  constructor(
    ...
    this.translate
      ...
      .subscribe((translatedMessages: any) => {
        ...
        this.customerConfigAlertMessage =
          translatedMessages['pw.tep.customerConfigAlertMessage'];
      });
  }
  ...
  openCustomerSearchAvailabilityValidationDialog() {
    this.alertPopupService
      .open({
        ...
        message: this.customerConfigAlertMessage
      })
      .pipe(takeUntil(this.destroy$))
  ...
  initiateTepOnInit() {
    ...
    this.commonFacade.setSelectedTepType(null);
    ...
    this.commonFacade.setTransactionConfig({
      transactionType: {
        ...
        subType: this.createTepFormGroup.get('tepType').value // NEW_TEP
      }
    });
    ...
    this.tepFacade
      ...
      .subscribe(data => {
        if (data) {
          ...
          this.totalTransactionAmt = data?.totalTxnAmt;
    ...
    if (id && this.transactionStatus === TepStatusEnum.APPROVED) {
      this.router.navigate([getCreateTepUrl('new')]);
    }
    ...
    const isCreateTepTabSelected = this.router.url.includes('create-tep');
    this.commonFacade.setFileUploadVisible(false);
    if (id !== 'new' && isCreateTepTabSelected) {
      this.tepItemList = [];
      this.clearTepData();
    ...
    this.router.events
      .pipe(
        filter((event: Event) => {
          return event instanceof NavigationEnd && this.isLoggedIn;
        }),
    ...
    this.getProductCodeCountForSeachedItem();
  ...
  updateTransactionDetailsOnScreen(
    ...
    if (
      ...
      if (
        tepTransactionDetails.refundDetails &&
        tepTransactionDetails.refundDetails.data &&
        tepTransactionDetails.refundDetails.data.refundMode &&
        tepTransactionDetails.refundDetails.data &&
        (tepTransactionDetails.refundDetails.data.refundMode.toUpperCase() ===
          'CHEQUE' ||
          tepTransactionDetails.refundDetails.data.refundMode.toUpperCase() ===
            'RTGS')
      ) {
        // this.tepFacade.loadFileIdProofDownloadUrl({
        //   txnType: TransactionTypeEnum.TEP,
        //   id: this.tepTransactionId,
        //   customerId: this.customerId,
        //   uploadType: TepUploadTypes.ID_PROOF
        // });
      }
      if (
        tepTransactionDetails.refundDetails &&
        ...
        // this.tepFacade.loadFileCancelledChequeDownloadUrl({
        //   txnType: TransactionTypeEnum.TEP,
        //   id: this.tepTransactionId,
        //   customerId: this.customerId,
        //   uploadType: TepUploadTypes.CANCELLED_CHEQUE
        // });
      }
      ...
      .subscribe((tepItemDetails: any) => {
        if (tepItemDetails) {
          ...
          const tepItemObject = {
            ...
            lotNumber: tepItemDetails.priceDetails.lotNumber,
  ...
  getTranslatedAlertMessages() {
    ...
    const selectApprovalDateAlertMsg = 'pw.tep.selectApprovalDateAlertMsg';
    this.translate
      .get([
        selectCustomerAlertMessage,
        selectRsoNameAlertMessage,
      ...
      .subscribe(translatedMessages => {
        ...
        this.selectApprovalDateAlertMsg =
          translatedMessages[selectApprovalDateAlertMsg];
      });
  }
        ...
        this.overlayNotificationServiceAbstraction
          ...
          .subscribe((event: OverlayNotificationEventRef) => {
            if (event.eventType === OverlayNotificationEventType.CLOSE) {
              if (
                this.openStatus === StatusTypesEnum.HOLD ||
                this.openStatus === StatusTypesEnum.OPEN
              ) {
                  ...
                  if (
                    ...
                    if (
                      (this.tepItemList.length === 1 &&
                        ...
                        !this.modalDetails.isExceptionScenario) ||
                      this.createTepFormGroup.get('tepType').value ===
                        CreateTepTypesEnum.FULL_VALUE_TEP
                    ) {
                      this.commonFacade.setIsTepRequestRaising(true);
                    ...
                    const addTepItemRequestPayload: AddTepItemRequestPayload = {
                      ...
                      totalWeight: Number(modalData.totalWeight.toFixed(3)),
    ...
    this.tepFacade
      ...
      .subscribe((addTepItemResponse: AddOrUpdateTepItemResponse) => {
        if (addTepItemResponse && !this.isSaleableModified) {
          ...
          if (CreateTepTypesEnum.FULL_VALUE_TEP) {
            // this.addedItemCmDocDate = moment(
            //   addTepItemResponse.itemDetails.refDocDate
            // );
        ...
        if (
          this.tepItemList.length === 1 &&
          addTepItemResponse.itemDetails.priceDetails &&
          addTepItemResponse.itemDetails.priceDetails.cmDocNo &&
          this.studdedProductGroupCodes.includes(
            addTepItemResponse.itemDetails.priceDetails.productGroupCode
          ) &&
          !addTepItemResponse.itemDetails.priceDetails.lotNumber
        ) {
          this.commonFacade.setIsTepRequestRaising(true);
          this.tepFacade.loadTepItemExceptionConfiguration(
            this.tepItemList[0].variantCode,
      ...
      .subscribe((response: DeleteTepItemResponse) => {
        if (response) {
          ...
          if (this.tepItemList.length === 0) {
            ...
            this.createTepFormGroup.get('fvtApprovalCode').setValue(null);
  ...
  confirmOrHoldTep(
    ...
    if (dummyTepItemRows && dummyTepItemRows.length > 0) {
      ...
      this.createTepFormGroup.get('tepType').value ===
        CreateTepTypesEnum.FULL_VALUE_TEP &&
      this.approverProcessType === GrnApprovalTypeEnums.EMAIL &&
      !this.createTepFormGroup.get('fvtApprovalDate').value
    ) {
      this.showAlertNotification(this.selectApprovalDateAlertMsg);
    } else if (
      this.createTepFormGroup.get('tepType').value ===
        CreateTepTypesEnum.FULL_VALUE_TEP &&
      ...
      if (
        ...
        refundDetails = {
          ...
          data: {
            refundMode: this.refundDetailsFromRefundForm?.paymentSubRefundModes
              ? this.refundDetailsFromRefundForm?.paymentSubRefundModes
              : this.refundDetailsFromRefundForm?.refundTypeSelected,
  ...
  getSelectedTepType(event) {
    if (event.value === CreateTepTypesEnum.FULL_VALUE_TEP) {
      this.commonFacade.setIsTepRequestRaising(true);
    } else {
      this.commonFacade.setIsTepRequestRaising(false);
    }
  ...
  getSelectedType(event: any) {
    this.commonFacade.setSelectedTepType(event);
    this.selectedTepType = event;
  }
  ...
  printTepAnnexure(transactionId: string) {
    this.isLastTransactionPrint = false;
    this.printingService.loadPrintData({
  ...
  print() {
    ...
    combineLatest([
      this.printingFacade.getLastTransactionId(),
      this.printingFacade.getLastTransactionPaymentType()
    ])
      .pipe(takeUntil(this.destroy$))
      .subscribe(([lastTransactionId, lastTransactionPaymentType]) => {
          ...
          this.postConfirmationActions
            ...
            .subscribe(res => {
              if (res) {
                ...
                this.printingService.loadPrintData({
                  printType:
                    lastTransactionPaymentType === TepPaymentTypesEnum.REFUND
                      ? printTypesEnum.TEP_REFUND
                      : printTypesEnum.TEP_PRINTS,
  ...
  onApproverSelectionChange(event: any) {
    ...
    if (this.tepItemList.length > 0) {
      ...
      if (this.approverProcessType === GrnApprovalTypeEnums.EMAIL) {
        this.commonFacade.setFileUploadVisible(true);
        console.log(this.approverProcessType, '  PROCESS TYPE');
        // this.showAlertNotification(
