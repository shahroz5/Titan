...
export class ResidualWeightConfigUiGridComponent
    ...
    this.columnDefs = [
      {
        ...
        width: 40,
        minWidth: 40 // suppressMovable: true
      },
    ...
    if (
      ...
      dialogRef.afterClosed().subscribe(res => {
        if (res) {
          ...
          //   rowNode.data.configValue = res.data.tolerance
          //     ? res.data.configValue
          //     : rowNode.data.configValue;
          //   rowNode.data.rangeId = res.data.rangeId
          //     ? res.data.rangeId
          //     : rowNode.data.rangeId;
          //   //  if (rowNode.data.id !== '') {
  ...
  onCellFocused(event) {
    ...
    if (this.currentRowField === 'rangeId') {
      this.currentRowInfo = this.getRange(
        this.gridApi.getValue(this.currentRowField, node)
      );
    } else {
