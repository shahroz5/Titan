...
export class ResidualWeightDetailsComponent implements OnInit, OnDestroy {
      ...
      .subscribe(param => {
        ...
        if (this.configId !== 'new') {
          this.locationMappingFacade.loadMappedLocations({
            ruleType: ConfigTypeEnum.ORDER_AB_RESIDUAL_TOLERANCE_CONFIG,
  ...
  back() {
    this.residualWeightConfigFacade.loadReset();
    this.router.navigate([getResidualWeightToleranceConfigRouteUrl()]);
  }
      ...
      .subscribe((event: OverlayNotificationEventRef) => {
        if (event.eventType === OverlayNotificationEventType.CLOSE) {
          this.router.navigate([
            getResidualWeightToleranceDetailsRouteUrl(this.configId)
          ]);
  ...
  openLocationMapping() {
    ...
    this.locationMappingService.open({
      ...
      configDetails: {
        ...
        configType: ConfigTypeEnum.ORDER_AB_RESIDUAL_TOLERANCE_CONFIG
      }
    });
  }
  updateConfig() {
  ...
  showLocMappingAlertMessage() {
    ...
    this.alertPopupService
      .open({
        ...
        message: 'pw.abResidualToleranceDetails.locationsMandatoryMessage'
      })
      .pipe(take(1))
      .subscribe();
  }
