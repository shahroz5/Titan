...
@Component({
  selector: 'poss-web-stock-receive-factory',
  templateUrl: './stock-receive-factory.component.html',
  styleUrls: ['./stock-receive-factory.component.scss']
})
export class StockReceiveFactoryComponent
  ...
  validateSearch(searchValue: string): boolean {
    if (this.stockReceiveType === StockReceiveTypesEnum.CFA_BTQ) {
      return fieldValidation.alphaNumericField.pattern.test(searchValue);
    } else {
  ...
  fetchSTNFromOracle() {
    this.stockReceiveFacade.fetchSTNFromOracle(
  ...
  clearSearch(): void {
    ...
    if (this.stockReceiveType !== StockReceiveTypesEnum.HISTORY) {
      ...
      if (this.hasFromFetchedOracle) {
        this.stockReceiveFacade.clearStocks();
        this.isPendingFactorySTNLoadedOnce = false;
        this.loadStocks(0);
      }
    } else {
      this.docNumber = null;
  ...
  loadStocks(pageIndex: number): void {
    if (this.isLoadingPendingFactorySTN === false) {
  ...
  onSelected(stock: any): void {
    this.router.navigate([
      getStockReceiveRouteUrl(StockReceiveTypesEnum.FAC_BTQ),
      stock.id,
