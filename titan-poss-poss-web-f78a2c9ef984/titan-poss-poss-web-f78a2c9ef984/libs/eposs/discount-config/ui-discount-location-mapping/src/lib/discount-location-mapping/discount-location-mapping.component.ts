...
@Component({
  selector: 'poss-web-discount-location-mapping',
  templateUrl: './discount-location-mapping.component.html',
  styleUrls: ['./discount-location-mapping.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class DiscountLoactionMappingComponent
  implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  @Input() mappedLocation;
  @Input() levels: [] = [];
  @Input() isPreviewApplicable;
  @Input() isDateRangeApplicable;
  ...
  currentDate = moment();
  component: DiscountLoactionMappingComponent = this;
  ...
  ngOnInit() {
    console.log(this.isDateRangeApplicable);
    this.translate
      .get([
      ...
      .subscribe((translatedMsg: any) => {
        this.columnDefs = [
          ...
          {
            headerName:
              translatedMsg['pw.discountLocationMapping.previewStartDateLabel'],
            field: 'previewStartDate',
            valueFormatter: params => {
              return this.dateFormatterService.format(params.value);
            },
            width: 140,
          ...
          {
            headerName:
              translatedMsg['pw.discountLocationMapping.previewEndDateLabel'],
            field: 'previewEndDate',
            valueFormatter: params => {
              return this.dateFormatterService.format(params.value);
            },
            width: 140,
          ...
          {
            headerName: translatedMsg['pw.discountLocationMapping.statusLabel'],
            field: 'isActive',
  ...
  gridReady(params: GridReadyEvent) {
    ...
    const previewStartColumn = this.columnApi
      .getAllColumns()
      .find(x => x.colDef.headerName === 'Preview Start Date');
    const previewEndColumn = this.columnApi
      .getAllColumns()
      .find(x => x.colDef.headerName === 'Preview End Date');
    if (!this.isPreviewApplicable)
      this.columnApi.setColumnsVisible(
        [previewStartColumn, previewEndColumn],
        false
      );
  }
  ...
  editDateRange() {
    if (
      ...
      if (!this.dupLocation) {
        this.dialog
          .open(DiscountDateRangeComponent, {
            width: '400px',
            autoFocus: false,
            disableClose: true,
            data: {
