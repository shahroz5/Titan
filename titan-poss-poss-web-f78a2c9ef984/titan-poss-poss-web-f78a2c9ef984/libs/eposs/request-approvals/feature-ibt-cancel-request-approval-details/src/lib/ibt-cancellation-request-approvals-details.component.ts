...
const BACK_SHORTCUT_KEY =
  'IbtCancellationRequestApprovalsDetailsComponent.BACK';
const componentName = 'IbtCancellationRequestApprovalsDetailsComponent';
@Component({
  selector: 'poss-web-ibt-cancellation-request-approvals-details',
  templateUrl: './ibt-cancellation-request-approvals-details.component.html'
})
export class IbtCancellationRequestApprovalsDetailsComponent
  ...
  ngOnInit() {
    ...
    this.ibtCancelCount$ = this.facade.getIbtCancelCount();

    this.Items$ = this.facade.getibtCancelItems();

    this.InterBoutiqueTransferStatusRequestNotifications();
    this.facade.loadSelectedCancelRequest({
      id: this.requestId,
      requestType: 'BTQ_BTQ'
    });
  ...
  initialLoad() {
    ...
    this.facade.loadIbtCancelRequestItems({
      id: this.id,
      requestType: 'BTQ_BTQ',
      pageIndex: this.pageIndex,
  ...
  shortcutEventHandler(command: Command): void {
    if (command.name === BACK_SHORTCUT_KEY) {
          ...
          .subscribe((event: OverlayNotificationEventRef) => {
            ...
            if (event.selectedId === 1) {
              ...
              const acceptStatus = {
                ...
                stUpdateDto: {
                  remarks: event.data,
                  status: 'CNCL_APVL_APPROVED'
                },
                transferType: 'BTQ_BTQ'
              };
              this.facade.updateIbtCancel(acceptStatus);
            }
            if (event.selectedId === 2) {
              const rejectStatus = {
                ...
                stUpdateDto: {
                  remarks: event.data,
                  status: 'CNCL_APVL_REJECTED'
                },
                transferType: 'BTQ_BTQ'
              };
              this.facade.updateIbtCancel(rejectStatus);
            }
          });
      });
  }
  ...
  InterBoutiqueTransferStatusNotifications(status: string) {
    ...
    let key: string;
    if (status === 'CANCELLED') {
      key = 'pw.interBoutiqueTransferNotifications.cancelApprovalIBt';
    } else {
      key = 'pw.interBoutiqueTransferNotifications.canceledApprovalIbt';
    }
    this.translate
      .get([key])
      .pipe(takeUntil(this.destroy$))
