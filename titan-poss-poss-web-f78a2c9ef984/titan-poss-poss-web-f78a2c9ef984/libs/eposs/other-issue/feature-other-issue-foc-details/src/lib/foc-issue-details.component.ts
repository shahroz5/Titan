...
const filterShortCutKey = 'FocIssueDetailsComponent.FILTER';
const sortShortCutKey = 'FocIssueDetailsComponent.SORT';
const backShortCutKey = 'FocIssueDetailsComponent.BACK';
const mainSearchShortCutKey = 'FocIssueDetailsComponent.MAIN_SEARCH';
const secondarySearchShortCutKey = 'FocIssueDetailsComponent.SECONADARY_SEARCH';
const clearAllShortCutKey = 'FocIssueDetailsComponent.CLEAR_ALL';
const selectAllShortCutKey = 'FocIssueDetailsComponent.SELECT_ALL';
const shortcutHelp = 'Global.SHORTCUT_HELP';
const componentName = 'FocIssueDetailsComponent';
@Component({
  selector: 'poss-web-foc-issue-details',
  templateUrl: './foc-issue-details.component.html',
  styleUrls: ['./foc-issue-details.component.scss']
})
export class FocIssueDetailsComponent implements OnInit, OnDestroy {
  destroy$: Subject<null> = new Subject<null>();
  otherIssuesTabEnumRef = OtherReceiptsIssuesEnum;
  onSearchFlag: boolean;
  cartCount = 0;
  searchCount: any;
  searchedFOCItems$: Observable<OtherIssuesItem[]>;
  ...
  itemIds = [];
  allItemIds = [];
  createStockRequestFOCResponse$: Observable<OtherIssueModel>;
  searchedFOCItemsInCart$: Observable<OtherIssuesItem[]>;
  searchedFOCItemsInCartResults: OtherIssuesItem[];
  FOCForm: FormGroup;
  isSearchingFOCItem$: Observable<boolean>;
  hasSearchedFOCItem$: Observable<boolean>;
  isloadingFOC$: Observable<boolean>;
  hasSearchItemInCartSearchFOC$: Observable<boolean>;
  ...
  componentInit() {
    this.searchedFOCItemsInCartResults = [];
    this.isSearchingFOCItem$ = this.otherIssueFacade.getIsSearchingFOC();
    this.hasSearchedFOCItem$ = this.otherIssueFacade.gethasSearchedItemFOC();
    this.isloadingFOC$ = this.otherIssueFacade.getIsLoadingFOC();
    this.hasSearchItemInCartSearchFOC$ = this.otherIssueFacade.getHasSearchItemInCartSearchFOC();
    ...
    this.searchedFOCItems$ = this.otherIssueFacade.getSearchedFOCItems();
    this.searchedFOCItems$.pipe(takeUntil(this.destroy$)).subscribe(items => {
    ...
    this.itemsInCart$ = this.otherIssueFacade.getFOCItemsInCarts();
    ...
    this.createStockRequestFOCResponse$ = this.otherIssueFacade.getCreateStockRequestFOCResponse();
    this.createStockRequestFOCResponse$
      .pipe(takeUntil(this.destroy$))
      .subscribe(data => {
        if (data.reqDocNo) {
    ...
    this.FOCForm.statusChanges.pipe(takeUntil(this.destroy$)).subscribe(() => {
      this.showNotification();
    });
  ...
  onSearch(searchResponse: ItemSearchResponse) {
    ...
    if (this.allItemsInCart.length < this.maxProductInList) {
      this.otherIssueFacade.searchFOCItems({
  ...
  addToCart(items: OtherIssuesItem[]) {
    ...
    if (
      this.allItemsInCart.length + this.items.length < this.maxProductInList &&
      this.onSearchFlag === true
    ) {
      this.searchListRef.reset();
      this.otherIssueFacade.addFOCItemsToCart(items);
    }
  }
          ...
          .subscribe((event: OverlayNotificationEventRef) => {
            ...
            if (event.eventType === OverlayNotificationEventType.TRUE) {
              if (this.FOCForm.invalid) {
                ...
                this.hasNotification = true;
                this.otherIssueFacade.createStockRequestFOC({
                  reqType: OtherReceiptsIssuesEnum.FOC,
                  approvalDetails: {
                    data: {
                      approvalCode: this.FOCForm.value.approvalCode,
                      approvedBy: this.FOCForm.value.approvedBy
                    },
                    type: 'approval'
                  },
  ...
  updateItem(itemToUpdate: PSVItemToUpdate) {
    this.otherIssueFacade.updateFOCCartItem({
      id: itemToUpdate.id,
    ...
    this.translate
      ...
      .subscribe((translatedMessage: string) => {
        this.overlayNotification
          ...
          .subscribe((event: OverlayNotificationEventRef) => {
            if (event.eventType === OverlayNotificationEventType.TRUE) {
              this.otherIssueFacade.removeSelectedFOCItems({
                ids:
                  this.selectForm.value.selectRadioButton === '1'
                    ? this.allItemIds
                    : this.itemIds
  ...
  back() {
    ...
    //   /* 'inventory/instock/other-receipts-issues-list/list/otherissues/Exhibition' */
  ...
  resetData() {
    this.otherIssueFacade.resetFOCIssueData();
  }
  clearSearchItems() {
    this.itemCode = null;
    this.lotNumber = null;
  ...
  clearAll() {
    ...
    this.otherIssueFacade.clearSearchCartItemFOC();
    this.overlayNotification.close();
    this.showNotification();
  }
  ...
  clearInventorySearch() {
    ...
    this.loadItemCart();
    this.otherIssueFacade.clearSearchInventoryItemFOC();
  }
