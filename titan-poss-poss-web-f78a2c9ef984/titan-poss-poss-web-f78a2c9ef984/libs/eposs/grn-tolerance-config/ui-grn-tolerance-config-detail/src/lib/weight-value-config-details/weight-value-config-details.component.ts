...
export class WeightValueConfigDetailsComponent
  ...
  save = true;
  weightBasedEditMode = false;
  valueBasedEditMode = false;
  ...
  errorMessageForCurrentEndRangeGreaterThanNextStartRange: string;
  errorMessageForCurrentEndRangeLessThanStartRange: string;
  errorMessageForCurrentStartRangeLessThanPreviousEndRange: string;
  ...
  ngOnInit() {
    ...
    this.weightValueConfigTotal$
      .pipe(takeUntil(this.destroy$))
      .subscribe(data => {
        if (data !== 0) {
          this.weightValueConfigDetailsForm.get('configName').clearValidators();
          this.weightValueConfigDetailsForm
            .get('configName')
            .setValidators([
              this.fieldValidatorsService.isUniqueCheck(this.configName, false)
            ]);

          this.weightValueConfigDetailsForm.controls[
            'configName'
          ...
          this.weightValueConfigDetailsForm
            ...
            .setValidators([
              ...
              this.fieldValidatorsService.nameWithSpaceField(this.configName)
            ]);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    this.routeParam.subscribe(param => {
      if (param['_id'] === WeightValueConfigConstants.NEW) {
        this.isNew = true;
        this.weightValueConfigDetailsForm?.get('configName').enable();
      } else {
        this.isNew = false;
        this.weightValueConfigDetailsForm?.get('configName').disable();
      }
      this.cdr.markForCheck();
    });
    this.save = true;
    if (changes['weightValueConfigDetails'] || changes['routeParam']) {
  ...
  initForm(weightValueConfigDetails: WeightValueConfigDetails) {
    this.weightValueConfigDetailsForm = new FormGroup({
      ...
      isActive: new FormControl(weightValueConfigDetails.isActive)
    });
  ...
  addAboveNewValueBasedMapping() {
    ...
    this.isAbove = true;
    if (this.valueBasedEditMode) {
      this.valueBasedEditMode = false;
      this.valueGridApi.stopEditing();
    } else {
  ...
  addAboveNewWeightBasedMapping() {
    ...
    if (this.weightBasedEditMode) {
      this.weightBasedEditMode = false;
  ...
  weightBasedRowEditingStarted(event) {
    this.weightBasedEditMode = true;
  ...
  valueBasedRowEditingStarted(event) {
    this.valueBasedEditMode = true;
  ...
  getWeightContext() {
    return {
      validators: {
        startRange: [this.fieldValidatorsService.requiredField('Start Range')],
        endRange: [this.fieldValidatorsService.requiredField('End Range')],
        tolerenceAllowed: [
          this.fieldValidatorsService.requiredField('Tolerance Allowed'),
          this.fieldValidatorsService.toleranceField('Tolerance Allowed')
        ]
      },
  ...
  getValueContext() {
    return {
      validators: {
        ...
        tolerenceAllowed: [
          this.fieldValidatorsService.requiredField('Tolerance Allowed'),
          this.fieldValidatorsService.percentageField('Tolerance Allowed')
        ]
      },
      componentParent: this.weightValueConfigDetailsComponent,
      focusOn: 'startRange'
  ...
  remove(params: CellEvent) {
    if (
      ...
      this.alertPopupService
        .open({
          type: AlertPopupTypeEnum.CONFIRM,
          message: 'pw.weightValueConfig.deleteConfimMsg'
        })
        .pipe(takeUntil(this.destroy$))
        .subscribe((res: boolean) => {
          if (res) {
  ...
  changeEvent(event) {
    this.weightValueConfigDetailsForm.patchValue({
  ...
  loadCheckConfigName() {
    ...
    if (
      this.weightValueConfigDetailsForm.get('configName').value !== '' &&
      this.weightValueConfigDetailsForm.get('configName').valid
    ) {
      if (
  ...
  showNotifications(errorKey: string) {
    const key = errorKey;
    this.translationService
      .get(key)
      .pipe(take(1))
