...
@Component({
  selector: 'poss-web-tep-exception-list',
  templateUrl: './tep-exception-list.component.html',
  styleUrls: ['./tep-exception-list.component.css']
})
export class TepExceptionListComponent implements OnInit,OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.filterForm = new FormGroup({
      ...
      type: new FormControl('TEP_EXCEPTION_APPROVAL_WORKFLOW')
    });
      ...
      filterOptions: {
        ...
        filterParams: {
          locationCode: this.filterForm.get('locationCode').value
            ...
            : undefined,
          mobileNumber: this.filterForm.get('mobileNumber').value
            ? this.filterForm.get('mobileNumber').value
            : undefined
        }
      }
    };
  ...
  clearmobileNumberSearch() {
    this.inValidSearch = false;
    this.pageIndex = 0;
    this.filterForm.get('mobileNumber').clearValidators();
    this.filterForm.get('mobileNumber').reset();
    this.loadList();
  }
  clearLocationSearch() {
    this.inValidSearch = false;
  ...
  cellClicked(data: tepApprovalListResponse) {
    if (data.tepExceptionDetails?.data) {
