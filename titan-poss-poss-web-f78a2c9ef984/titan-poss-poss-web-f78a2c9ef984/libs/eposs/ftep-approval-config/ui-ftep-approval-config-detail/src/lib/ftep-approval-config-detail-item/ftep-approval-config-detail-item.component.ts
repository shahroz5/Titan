...
@Component({
  selector: 'poss-web-ftep-approval-config-detail-item',
  templateUrl: './ftep-approval-config-detail-item.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class FtepApprovalConfigDetailItemComponent
  implements OnInit, OnDestroy, OnChanges {
  @Input() ftepApprovalConfig$: Observable<FtepApprovalConfigResponse>;
  @Input() roleList$: Observable<RoleList[]>;

  @Output() saveFtepApprovalConfig = new EventEmitter<FtepApprovalConfig>();
  ...
  ruleId: string;
  ruleType: string;
  ftepApprovalConfigForm: FormGroup;
  ...
  constructor(
    ...
    this.translateService
      .get([
        'pw.ftepApprovalConfig.approverLabel',
        'pw.ftepApprovalConfig.approveTypeLabel',
        'pw.ftepApprovalConfig.fromNumberLabel',
        'pw.ftepApprovalConfig.toNumberLabel',
        'pw.ftepApprovalConfig.upperLimitLabel'
      ])
  ...
  columnDefinitions(translatedMessages: any) {
    this.columnDefs = [
      {
        headerName: translatedMessages['pw.ftepApprovalConfig.approverLabel'],
        field: 'roleCode',
        editable: false,
        resizable: true,
        minWidth: 100
      ...
      {
        headerName:
          translatedMessages['pw.ftepApprovalConfig.approveTypeLabel'],
        field: 'processType',
        minWidth: 100,
      ...
      {
        headerName: translatedMessages['pw.ftepApprovalConfig.fromNumberLabel'],
        field: 'fromDays',
        resizable: true,
        editable: true,
      ...
      {
        headerName: translatedMessages['pw.ftepApprovalConfig.toNumberLabel'],
        field: 'tillDays',
        cellClass: 'pw-form-input-width',
        resizable: true,
        editable: true,
      ...
      {
        headerName: translatedMessages['pw.ftepApprovalConfig.upperLimitLabel'],
        field: 'upperLimit',
        cellClass: 'pw-form-input-width',
        isAmount: true,
        resizable: true,
    ...
    if (
      (typeof currentRowData?.processType === 'object'
        ? currentRowData?.processType.value === '' ||
          currentRowData?.processType.value === null ||
          currentRowData?.processType.value === undefined
        : currentRowData?.processType === '' ||
          currentRowData?.processType === null ||
          currentRowData?.processType === undefined) ||
  ...
  addData(roleCode) {
    if (
      ...
      this.ftepApprovalConfigForm.get('approvedBy').patchValue('');
      ...
      validators: {
        fromDays: [
          this.fieldValidatorsService.requiredField(
            this.translatedMessages['pw.ftepApprovalConfig.fromNumberLabel']
          ),
        ...
        tillDays: [
          this.fieldValidatorsService.requiredField(
            this.translatedMessages['pw.ftepApprovalConfig.toNumberLabel']
          ),
          this.fieldValidatorsService.daysField(
            this.translatedMessages['pw.ftepApprovalConfig.toNumberLabel']
          )
  ...
  initForm(ftepApprovalConfigDetails: FtepApprovalConfigResponse) {
    this.translateService
      .get([
        'pw.ftepApprovalConfig.ftepTypeLabel',
        'pw.ftepApprovalConfig.configNameLabel',
        'pw.ftepApprovalConfig.approveTypeLabel',
        'pw.ftepApprovalConfig.fromNumberLabel',
        'pw.ftepApprovalConfig.toNumberLabel'
      ])
      .pipe(takeUntil(this.destroy$))
      .subscribe((translatedMessages: any) => {
        this.ftepApprovalConfigForm = new FormGroup({
          configName: new FormControl(
            ftepApprovalConfigDetails.description
              ? ftepApprovalConfigDetails.description
              : '',
            [
              this.fieldValidatorsService.requiredField(
                translatedMessages['pw.ftepApprovalConfig.configNameLabel']
              ),
          ...
          isActive: new FormControl(
            ftepApprovalConfigDetails.isActive
              ? ftepApprovalConfigDetails.isActive
              : false
          ),
          approvedBy: new FormControl('')
        });
        if (ftepApprovalConfigDetails && ftepApprovalConfigDetails.config) {
          ftepApprovalConfigDetails.config.forEach((configDetails, index) => {
            this.ftepApprovalConfigForm.addControl(
              index.toString(),
              new FormGroup({
                ...
                tillDays: new FormControl(
                  ...
                  [
                    this.fieldValidatorsService.requiredField(
                      translatedMessages['pw.ftepApprovalConfig.toNumberLabel']
                    )
                  ]
                ),
    ...
    if (this.ruleId !== 'new') {
      ...
      this.ftepApprovalConfigForm.get('configName').disable();
    }
  ...
  ngOnInit() {
    ...
    this.roleList$.pipe(takeUntil(this.destroy$)).subscribe(roleList => {
      if (roleList) {
      ...
      if (
        ...
        if (changeEvent.id !== '') {
          switch (changeEvent.colDef.field) {
            case 'processType': {
              this.ftepApprovalConfigForm.patchValue({
  ...
  saveDetail() {
    if (this.configDetail?.description !== '' && !this.configDetail?.isActive) {
      ...
      if (this.ftepApprovalConfigForm.valid && this.getAllRows().length > 0) {
        this.api.stopEditing();
        this.addRowData = false;
            ...
            .subscribe((res: boolean) => {
              if (res) {
                if (
                  ...
                  this.saveFtepApprovalConfig.emit(payload);
                }
              }
            });
        }
  ...
  prepareResponse() {
    const addFtepDetails = [];
    for (const ftep of this.getAllRows()) {
      if (ftep.id === '' || ftep.id === undefined || ftep.id === null) {
        if (ftep.upperLimit) {
          addFtepDetails.push({
            roleCode:
              typeof ftep.roleCode === 'object'
                ? ftep.roleCode.value
                : ftep.roleCode,

            processType:
              typeof ftep.processType === 'object'
                ? ftep.processType.value
                : ftep.processType,
            fromDays:
              typeof ftep.fromDays === 'object'
                ? Number(ftep.fromDays.value)
                : Number(ftep.fromDays),
            tillDays:
              typeof ftep.tillDays === 'object'
                ? Number(ftep.tillDays.value)
                : Number(ftep.tillDays),
            upperLimit:
              typeof ftep.upperLimit === 'object'
                ? ftep.upperLimit.value
                  ? Number(ftep.upperLimit.value)
                  : null
                : Number(ftep.upperLimit)
          });
        } else {
          addFtepDetails.push({
            roleCode:
    ...
    const formValues = this.ftepApprovalConfigForm.getRawValue();
    const saveFtepApprovalConfig = {
      description: formValues.configName,
      isActive: formValues.isActive,
      ruleType: ConfigTypeEnum.FTEP_APPROVAL_ACCESS_REGULAR,
      ruleDetails: {
        data: {
          config: addFtepDetails
        },
        type: ConfigTypeEnum.FTEP_APPROVAL_ACCESS_REGULAR
      }
    };
    console.log(saveFtepApprovalConfig, 'saveFtepApprovalConfig');

    return saveFtepApprovalConfig;
  }
