...
const RSOCode = 'RSO';
@Component({
  selector: 'poss-web-grf-request-details',
  templateUrl: './grf-request-details.component.html',
  styleUrls: ['./grf-request-details.component.scss'],
})
export class GrfRequestDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    //     transactionType: {
    //       type: TransactionTypeEnum.GRF,
    //       subType: SubTransactionTypeEnum.MANUAL_GRF
    //     },
    //     workflowData: {
  ...
  confirmCashmemo(event, data) {
    const orderDetails: UpdateOrderDetails = {
      ...
      subTransactionType: SubTransactionTypeEnum.MANUAL_GRF
    };

    const msg = this.orderService.confirmOrder(
      orderDetails,
      this.paymentDetails,
      TransactionTypeEnum.GRF
    );
