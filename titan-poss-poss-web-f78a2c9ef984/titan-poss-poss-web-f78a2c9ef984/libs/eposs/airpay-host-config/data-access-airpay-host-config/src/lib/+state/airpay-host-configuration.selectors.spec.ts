import {
  CustomErrors,
  AirpayHostSuccessList,
  SortItem,
  ListingPayload,
  FileUploadResponse,
  HostNameList
} from '@poss-web/shared/models';
import { AirpayHostConfigEntity } from './airpay-host-configuration.entity';
import { AirpayHostConfigurationState } from './airpay-host-configuration.state';
import { initialState } from './airpay-host-configuration.reducer';
import * as selectors from './airpay-host-configuration.selectors';
import { Moment } from 'moment';

describe('Unipay Access Mapping Selector Testing Suite', () => {
  const formData: FormData = new FormData();
  const uploadResponse: FileUploadResponse = {
    fileId: 'test123',
    hasError: false,
    message: 'uploaded',
    records: {
      errorLogId: 'abc123',
      failureCount: 0,
      successCount: 1,
      totalCount: 1
    }
  };

  const configListingPayload: ListingPayload = {
    pageIndex: 0,
    pageSize: 10
  };
  const sortField: SortItem = {
    colId: '',
    sort: 'Desc'
  };

  const unipayConfigurationList1: AirpayHostSuccessList = {
    hostList: [
      {
        hostName: 'unipay',
        id: '123',
        isActive: true,
        locationCode: '123',
        newlyAdded: true,
        paymentCode: 'unipay'
      }
    ],
    count: 1
  };
  const accessList1: HostNameList = {
    hostName: 'unipay',
    id: '123',
    isActive: true,
    locationCode: '123',
    newlyAdded: true,
    paymentCode: 'unipay'
  };

  const accessList2: HostNameList = {
    hostName: 'unipay',
    id: '456',
    isActive: true,
    locationCode: '123',
    newlyAdded: true,
    paymentCode: 'unipay'
  };

  const accessListArray = [accessList1, accessList2];

  const addAccessListToEntities = <T extends HostNameList>(
    payload: T[]
  ): { [id: string]: T } => {
    const reducedEntities = payload.reduce(
      (entities: { [id: string]: T }, element: T) => {
        return {
          ...entities,
          [element.id]: element
        };
      },
      {}
    );
    return reducedEntities;
  };

  const accessElemets: AirpayHostConfigEntity = {
    ids: [accessList1.id, accessList2.id],
    entities: addAccessListToEntities(accessListArray)
  };

  describe('Testing Unipay Aceess Mapping Related selectors', () => {
    it('Should return the error', () => {
      const error: CustomErrors = {
        code: 'ERR_1',
        message: 'Error',
        traceId: 'TraceID',
        timeStamp: '122131',
        error: null
      };
      const state: AirpayHostConfigurationState = {
        ...initialState,
        hasError: error
      };
      expect(
        selectors.AirpayHostConfigurationSelectors.selectHasError.projector(
          state
        )
      ).toEqual(error);
    });

    // it('Should return the error', () => {
    //   const state: AirpayHostConfigurationState = {
    //     ...initialState,
    //     errorLog: []
    //   };
    //   expect(
    //     selectors.AirpayHostConfigurationSelectors.selectIsErrorLog.projector(
    //       state
    //     )
    //   ).toEqual([]);
    // });

    // it('Should return uploadResponse', () => {
    //   const state: AirpayHostConfigurationState = {
    //     ...initialState,
    //     fileUploadResponse: uploadResponse
    //   };
    //   expect(
    //     selectors.AirpayHostConfigurationSelectors.selectFileUploadResponse.projector(
    //       state
    //     )
    //   ).toEqual(uploadResponse);
    // });

    it('Should return selectAccessList', () => {
      // const state: AirpayHostConfigurationState = {
      //   ...initialState,
      //   accessList: accessElemets
      // };
      expect(
        selectors.AirpayHostConfigurationSelectors.selecthostNameList.projector(
          accessElemets
        )
      ).toEqual(accessListArray);
    });

    it('Should return selectTotalElements', () => {
      const state: AirpayHostConfigurationState = {
        ...initialState,
        totalCount: 1
      };
      expect(
        selectors.AirpayHostConfigurationSelectors.selectTotalElements.projector(
          state
        )
      ).toEqual(1);
    });

    it('should return isLoading selector', () => {
      const state: AirpayHostConfigurationState = {
        ...initialState,
        isLoading: true
      };
      expect(
        selectors.AirpayHostConfigurationSelectors.selectIsLoading.projector(
          state
        )
      ).toEqual(true);
    });

    it('should return updateResponse selector', () => {
      const state: AirpayHostConfigurationState = {
        ...initialState,
        updatedHostNameList: 'test'
      };
      expect(
        selectors.AirpayHostConfigurationSelectors.updateResponse.projector(state)
      ).toEqual('test');
    });
  });
});
