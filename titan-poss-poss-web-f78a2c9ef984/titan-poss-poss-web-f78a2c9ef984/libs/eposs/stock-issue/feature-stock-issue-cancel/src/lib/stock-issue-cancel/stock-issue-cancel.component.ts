...
@Component({
  selector: 'poss-web-stock-issue-cancel',
  templateUrl: './stock-issue-cancel.component.html',
  styleUrls: ['./stock-issue-cancel.component.scss']
})
export class StockIssueCancelComponent
  ...
  totalIssueCancelSTNCount$: Observable<number>;
      ...
      .subscribe((isCancellationAllowed: string) => {
        ...
        if (this.isCancellationAllowed) {
          this.router.navigate([
            getStockIssueRouteUrl(StockIssueTypesEnum.CANCEL)
          ]);
        } else {
          this.router.navigate([
        ...
        .subscribe((event: any) => {
          ...
          if (searchValue !== '') {
            if (this.validateSearch(searchValue)) {
              ...
              this.stockIssueFacade.clearPendingIssuesForCancel();
            }
          } else {
            this.clearSearch();
          }
  ...
  getIssues() {
    if (this.isL1L2Store) {
      this.BTQ_BTQ_IssuePendingSTNCount$ = this.stockIssueFacade.getPendingBTQ_BTQ_STNCancelCount();
      this.totalIssueCancelSTNCount$ = this.stockIssueFacade.getTotalIssueCancelSTNCount();
      this.BTQ_BTQ_PendingSTN$ = this.stockIssueFacade.getBTQ_BTQ_PendingSTNCancel();
  ...
  searchIssues(reqDocNo: number) {
    ...
    requestType = StockIssueAPIRequestTypesEnum.BTQ_BTQ;
    if (this.isL1L2Store) {
      this.stockIssueFacade.loadIssueToBoutiqueSTNCancel({
        requestType: StockIssueAPIRequestTypesEnum.BTQ_BTQ,
        srcDocNo: reqDocNo
      });
    }
  ...
  loadStocks(pageIndex) {
    if (!this.isLoading && this.isCancellationAllowed) {
      this.stockIssueFacade.loadIssueToBoutiqueSTNCancel({
        requestType: StockIssueAPIRequestTypesEnum.BTQ_BTQ,
        pageIndex: pageIndex,
  ...
  onSelected(issue: any) {
    if (this.isL1L2Store) {
      if (this.type !== StockIssueTypesEnum.HISTORY) {
        this.router.navigate(
          [getStockIssueCancelRouteUrl(StockIssueTypesEnum.BOUTIQUE, issue.id)],
          {
