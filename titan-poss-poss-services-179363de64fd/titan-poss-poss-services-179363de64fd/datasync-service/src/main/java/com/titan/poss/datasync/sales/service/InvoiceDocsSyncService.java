...
@Service
@Slf4j
public class InvoiceDocsSyncService implements SyncOperation {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(InvoiceDocsSyncService.class);
	...
	private Boolean syncConfirmData(List<SyncData> syncData) {
		List<SalesInvoiceDocumentsDao> invoiceDocs = new ArrayList<>();
		ObjectMapper mapper = new ObjectMapper();
		for (SyncData data : syncData) {
			if (data.getOrder() == 0) {
				salesCommon.syncInvoiceDocs(data, invoiceDocs, mapper);
		     }
		}
		boolean flag=false;
		if (!invoiceDocs.isEmpty()) {
			salesInvoiceDocumentsRepository.saveAll(invoiceDocs);
			flag = true;
		}
		return flag;
	}
