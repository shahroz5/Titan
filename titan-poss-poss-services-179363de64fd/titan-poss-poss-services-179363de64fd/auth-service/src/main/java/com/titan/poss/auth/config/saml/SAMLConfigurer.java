...
public class SAMLConfigurer extends SecurityConfigurerAdapter<DefaultSecurityFilterChain, HttpSecurity> {
	...
	private WebSSOProfileConsumerImpl webSSOProfileConsumer = webSSOProfileConsumer();
	...
	private static String serverServletPath;
	private static final Logger LOGGER = LoggerFactory.getLogger(SAMLConfigurer.class);
	static {
		try {
			Resource resource = new ClassPathResource("/application.properties");
			Properties props = PropertiesLoaderUtils.loadProperties(resource);
//			serverServletPath = props.getProperty("server.servlet-path");
			serverServletPath = props.getProperty("spring.mvc.servlet.path");
		} catch (IOException e) {
	...
	public void init(HttpSecurity http) {
		...
		http.addFilterBefore(metadataGeneratorFilter(samlEntryPoint, extendedMetadata), ChannelProcessingFilter.class)
				.addFilterAfter(
						samlFilter(samlEntryPoint, samlLogoutFilter, samlLogoutProcessingFilter, contextProvider),
	...
	private CachingMetadataManager cachingMetadataManager() {
		...
		CachingMetadataManager cachingManager = null;
		try {
			cachingManager = new CachingMetadataManager(providers);
			cachingManager.setKeyManager(serviceProvider.keyManager);
		} catch (MetadataProviderException e) {
			LOGGER.error("MetadataProviderException ", e);
		}
	...
	private StaticBasicParserPool staticBasicParserPool() {
		...
		try {
			...
			LOGGER.error(" XMLParserException ", e);
		}
		return parser;
	}

	private ExtendedMetadataDelegate extendedMetadataDelegate(ExtendedMetadata extendedMetadata) {
		ExtendedMetadataDelegate extendedDelegate = new ExtendedMetadataDelegate(metadataProvider, extendedMetadata);
		extendedDelegate.setMetadataTrustCheck(false);
		extendedDelegate.setMetadataRequireSignature(false);
		extendedDelegate.setRequireValidMetadata(false);
	...
	private WebSSOProfileOptions webSSOProfileOptions() {
		WebSSOProfileOptions webProfileOptions = new WebSSOProfileOptions();
		webProfileOptions.setIncludeScoping(false);
	...
	private SAMLProcessingFilter samlWebSSOProcessingFilter(SAMLAuthenticationProvider samlAuthenticationProvider,
			SAMLContextProvider contextProvider, SAMLProcessor samlProcessor) throws Exception {
		try {
			...
			throw new ServiceException("Unable to process SAML request", "ERR-AUTH-020");
	...
	private FilterChainProxy samlFilter(SAMLEntryPoint samlEntryPoint, SAMLLogoutFilter samlLogoutFilter,
		...
		chains.add(new DefaultSecurityFilterChain(new AntPathRequestMatcher("/saml/metadata/**"),
				metadataDisplayFilter(contextProvider))); // HotFix
		try {
	...
	private SingleLogoutProfile singleLogoutProfile() {
		...
		logoutProfile.setMetadata(cachingMetadataManager);
		logoutProfile.setProcessor(samlProcessor);
		logoutProfile.setMaxAssertionTime(60000); // HotFix
		logoutProfile.setResponseSkew(60000); // HotFix
		return logoutProfile;
	}

	private WebSSOProfileConsumerImpl webSSOProfileConsumer() {
		WebSSOProfileConsumerImpl webProfileConsumer = new WebSSOProfileConsumerImpl();
		webProfileConsumer.setMaxAssertionTime(60000); // HotFix
		webProfileConsumer.setResponseSkew(60000); // HotFix
		return webProfileConsumer;
	}
	...
	private SAMLAuthenticationProvider samlAuthenticationProvider(WebSSOProfileConsumerImpl webSSOProfileConsumer) {
		...
		samlProvider.setForcePrincipalAsString(false);
		samlProvider.setSamlLogger(samlLogger);
		samlProvider.setConsumer(webSSOProfileConsumer);
		samlProvider.setUserDetails(this.samlUserDetailsService);
		return samlProvider;
	}
	...
	public class ServiceProvider {
		...
		public ServiceProvider entityBaseURL(String entityBaseURL) {
		...
		private KeyManager keyManager() {
			...
			if (keyStore.getStoreFilePath().startsWith("file://")) {
				try {
					...
					LOGGER.error(" IOException ", e);
					throw new ServiceException("Cannot load file system resource: " + keyStore.getStoreFilePath(),
							"ERR-CORE-020");
				}
			}
		...
		public class KeyStore {
			...
			public String toString() {
				return "KeyStore{" + "storeFilePath='" + storeFilePath + '\'' + ", keyname='" + keyname + '\'' + '}';
			}
			...
			public boolean equals(Object o) {
				...
				if (o == null || getClass() != o.getClass())
					return false;
				KeyStore keyStoreLocal = (KeyStore) o;
				if (storeFilePath != null ? !storeFilePath.equals(keyStoreLocal.storeFilePath)
						: keyStoreLocal.storeFilePath != null)
					return false;
				if (password != null ? !password.equals(keyStoreLocal.password) : keyStoreLocal.password != null)
					return false;
				if (keyname != null ? !keyname.equals(keyStoreLocal.keyname) : keyStoreLocal.keyname != null)
					return false;
				return keyPassword != null ? keyPassword.equals(keyStoreLocal.keyPassword)
						: keyStoreLocal.keyPassword == null;
			}
