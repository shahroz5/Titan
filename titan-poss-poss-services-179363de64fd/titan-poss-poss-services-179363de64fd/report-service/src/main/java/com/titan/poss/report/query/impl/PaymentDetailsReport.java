 ...
 * @version 1.0
 */
@Service("paymentDetailsReport")
public class PaymentDetailsReport extends IReport {

	/**
	 * @param reportFactory
	 */
	public PaymentDetailsReport(ReportFactory reportFactory) {
		reportFactory.registerReportQuery("PAYMENT_DETAIL_REPORT", this);
	}
	...
	public String buildQuery(ReportRequestDto reportRequestDto) {
		...
		String commonWhereQuery = "WHERE pd.sales_txn_id  = st.id   AND st.location_code  = bmv.location_code and bmv.is_migrated_from_Legacy=1 and st.txn_source is null  AND pd.status ='COMPLETED' ";
