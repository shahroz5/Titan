...
@Component
public class GeneralLedgerCreditNoteReceiveL3RowMapper implements RowMapper<GeneralLedgerDto> {
	...
	public GeneralLedgerDto mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		String attribute5 = rs.getString("src_location_code");
		generalLedger.setAttribute5(attribute5);
		generalLedger.setAttribute6("");
		generalLedger.setAttribute4("");
		generalLedger.setAttribute3(rs.getString("src_location_code"));
		String attribute2 = StringUtils.isEmpty(rs.getString("debit_amount")) ? rs.getString("type_of_transaction")
				: rs.getString("type_of_transaction") + "-" + rs.getString("src_location_code");
