...
public class GeneralLedgerTepCashRefundRowMapper implements RowMapper<GeneralLedgerDto> {
	...
	public GeneralLedgerDto mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		String attribute5 = StringUtils.isEmpty(rs.getString("debit_amount")) ? rs.getString("location_code") + String.join("", Lists.reverse(
				Arrays.stream(rs.getDate("business_date").toString().trim().split("-")).collect(Collectors.toList()))) + "_Cash"
				: "Cash Refund_TEP";
		...
		String attribute2 = "Cash Refund_TEP-" + rs.getString("doc_date");
