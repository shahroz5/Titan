...
public class TcsReversalDebitNoteRowMapper  implements RowMapper<DebitNoteDto> {
...
private static final String AMOUNT = "tcs_amount_paid";
	...
	public DebitNoteDto mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		debitNoteDto.setReference(rs.getString(FileIntegrationConstants.LOCATION_CODE) + rs.getString("fiscal_year")
			...
			+ "CM" + rs.getString("doc_no") + "/");
			debitNoteDto.setTransactionType("TCSREVERSAL");
		...
		debitNoteDto.setMemoLine("TCS Reversed");
		debitNoteDto.setQty(1);
		if(rs.getString("cancellation_type").equalsIgnoreCase("REGULAR_GRN"))
			debitNoteDto.setPurchaseOrder("GRN" + rs.getString("doc_no"));
			else
		debitNoteDto.setPurchaseOrder("BC" + rs.getString("doc_no"));
