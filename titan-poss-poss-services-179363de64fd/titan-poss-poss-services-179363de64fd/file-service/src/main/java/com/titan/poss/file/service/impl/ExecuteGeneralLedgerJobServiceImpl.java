...
@Component
public class ExecuteGeneralLedgerJobServiceImpl implements ExecuteJobService {

	public ExecuteGeneralLedgerJobServiceImpl(JobFactory jobFactory) {
		jobFactory.registerJob(FileIntegrationConstants.GENERAL_LEDGER_JOB, this);
	}
	...
	private static final String GENERAL_LEDGER_LOCAL_FILE_PATH = "general.ledger.completed.path";

	private static final String GENERAL_LEDGER_LOCAL_ENCRYPTED_PATH = "general.ledger.encrypted.path";

	private static final String GENERAL_LEDGER_SFTP_SERVER_PATH = "sftp.general.ledger.path";
	...
	public SchedulerResponseDto triggerJob(LaunchJobRequest launchJobRequest) {
		...
		String localPath = localFileBasePath + env.getProperty(GENERAL_LEDGER_LOCAL_FILE_PATH);
		String sftpServerPath = env.getProperty(GENERAL_LEDGER_SFTP_SERVER_PATH);
		String localEncryptedPath = localFileBasePath + env.getProperty(GENERAL_LEDGER_LOCAL_ENCRYPTED_PATH);

		// upload files to sftp server
		FileMasterDao fileMaster = fileMasterRepository.findByFileGroupAndFileName(
				FileGroupEnum.ORACLE.toString(), FileMasterJobNameEnum.GENERAL_LEDGER_JOB.getValue());
		...
		response.setCode(SchedulerCodeEnum.FILE_GENERAL_LEDGER_JOB.toString());
		return response;
	}
}
