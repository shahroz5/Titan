...
@Component
public class ExecuteDebitNoteL3JobServiceImpl implements ExecuteJobService{

	public ExecuteDebitNoteL3JobServiceImpl(JobFactory jobFactory) {
		jobFactory.registerJob(FileIntegrationConstants.DEBIT_NOTE_L3_JOB, this);
	}
	...
	private static final String DEBIT_NOTE_LOCAL_FILE_PATH = "debit.note.lthree.completed.path";

	private static final String DEBIT_NOTE_LOCAL_ENCRYPTED_PATH = "debit.note.lthree.encrypted.path";

	private static final String DEBIT_NOTE_SFTP_SERVER_PATH = "sftp.debit.note.lthree.path";
	...
	public SchedulerResponseDto triggerJob(LaunchJobRequest launchJobRequest) {
		...
		FileMasterDao fileMaster = fileMasterRepository.findByFileGroupAndFileName(FileGroupEnum.ORACLE.toString(),
				FileMasterJobNameEnum.DEBIT_NOTE_L3_JOB.getValue());
		...
		response.setCode(SchedulerCodeEnum.FILE_DEBIT_NOTE_L3_JOB.toString());
		return response;
	}
