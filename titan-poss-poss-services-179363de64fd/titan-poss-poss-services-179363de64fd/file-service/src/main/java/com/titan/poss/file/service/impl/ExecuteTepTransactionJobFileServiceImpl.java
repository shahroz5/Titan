 ...
 */
@Component
public class ExecuteTepTransactionJobFileServiceImpl implements ExecuteJobService {
	
	public ExecuteTepTransactionJobFileServiceImpl(JobFactory jobFactory) {
		jobFactory.registerJob(FileIntegrationConstants.TEP_TRANSACTION_JOB, this);
	}
	...
	private static final String TEP_TRANSACTION_LOCAL_FILE_PATH = "tep.transaction.completed.path";

	private static final String TEP_TRANSACTION_LOCAL_ENCRYPTED_PATH = "tep.transaction.encrypted.path";

	private static final String TEP_TRANSACTION_SFTP_SERVER_PATH = "sftp.tep.transaction.path";
	...
	public SchedulerResponseDto triggerJob(LaunchJobRequest launchJobRequest) {
		...
		String localPath = localFileBasePath + env.getProperty(TEP_TRANSACTION_LOCAL_FILE_PATH);
		String sftpServerPath = env.getProperty(TEP_TRANSACTION_SFTP_SERVER_PATH);
		String localEncryptedPath = localFileBasePath + env.getProperty(TEP_TRANSACTION_LOCAL_ENCRYPTED_PATH);

		// upload files to sftp server
		FileMasterDao fileMaster = fileMasterRepository.findByFileGroupAndFileName(
				FileGroupEnum.ORACLE.toString(), FileMasterJobNameEnum.TEP_TRANSACTION_JOB.getValue());
		...
		response.setCode(SchedulerCodeEnum.FILE_TEP_TRANSACTION_JOB.toString());
		return response;
	}
