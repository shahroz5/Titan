...
@Component
public class ExecuteBoutiqueSaleJobServiceImpl implements ExecuteJobService {

	public ExecuteBoutiqueSaleJobServiceImpl(JobFactory jobFactory) {
		jobFactory.registerJob(FileIntegrationConstants.BOUTIQUE_SALES_JOB, this);
	}
	...
	private static final String BOUIQUE_SALES_LOCAL_FILE_PATH = "boutique.sales.completed.path";

	private static final String BOUIQUE_SALES_LOCAL_ENCRYPTED_PATH = "boutique.sales.encrypted.path";

	private static final String BOUIQUE_SALES_SFTP_SERVER_PATH = "sftp.boutique.sales.path";
	...
	public SchedulerResponseDto triggerJob(LaunchJobRequest launchJobRequest) {
		...
		String localPath = localFileBasePath + env.getProperty(BOUIQUE_SALES_LOCAL_FILE_PATH);
		String sftpServerPath = env.getProperty(BOUIQUE_SALES_SFTP_SERVER_PATH);
		String localEncryptedPath = localFileBasePath + env.getProperty(BOUIQUE_SALES_LOCAL_ENCRYPTED_PATH);

		// upload files to sftp server
		FileMasterDao fileMaster = fileMasterRepository.findByFileGroupAndFileName(
				FileGroupEnum.ORACLE.toString(), FileMasterJobNameEnum.BOUTIQUE_SALES_JOB.getValue());
		...
		response.setCode(SchedulerCodeEnum.FILE_BOUTIQUE_SALES_JOB.toString());
		return response;
	}
}
