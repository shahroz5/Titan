...
@Slf4j
@Component
public class NcMemberDataFileWritingStepListener implements StepExecutionListener {
	...
	private static final String MOVE_MEMBER_DATA_TO_AUDIT = "INSERT into ulp_member_data_aud select * from ulp_member_data_stage where file_audit_id = '";
	...
	public ExitStatus afterStep(StepExecution stepExecution) {
		String fileAuditId = stepExecution.getJobExecution().getExecutionContext().getString("NcMemberDataSavedId");
		...
		if (status.toString().equalsIgnoreCase("COMPLETED")) {
			...
			String locationCodeAndBusinessDateUpdateSql = "SELECT store_code as location_code, max(transaction_date) as business_date from [file].dbo.ulp_member_data_stage"
					...
					+ FileMasterJobNameEnum.NC_MEMBER_DATA_JOB.getValue() + "')) group by store_code;";

			String locationCodeAndBusinessDateInsertSql = "SELECT store_code as location_code ,max(transaction_date) as business_date from [file].dbo.ulp_member_data_stage "
					+ "where file_audit_id = '" + fileAuditId
			...
			fileService.updateFileMasterLocationMapping(locationCodeAndBusinessDateUpdateSql,
					locationCodeAndBusinessDateInsertSql, FileMasterJobNameEnum.NC_MEMBER_DATA_JOB.getValue(),
					fileAuditId);
			...
			jdbcTemplate.execute(MOVE_MEMBER_DATA_TO_AUDIT + fileAuditId + "'");
			log.debug("member data stage data moved to audit table");
		} else {
			fileAuditService.updateFileAudit(fileAuditId, status.toString(), FileGroupEnum.NETCARROTS.toString(),
					FileIntegrationConstants.FAILURE_MSG, null, false, sequenceNo, null);
		}
		log.debug("NcMemberData job is completed at " + CalendarUtils.getCurrentDate());
