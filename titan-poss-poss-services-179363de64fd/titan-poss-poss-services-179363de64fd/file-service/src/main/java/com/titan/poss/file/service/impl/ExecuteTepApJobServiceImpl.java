 ...
 */
@Component
public class ExecuteTepApJobServiceImpl implements ExecuteJobService {
	
	public ExecuteTepApJobServiceImpl(JobFactory jobFactory) {
		jobFactory.registerJob(FileIntegrationConstants.TEP_AP_JOB, this);
	}
	...
	private static final String TEP_AP_LOCAL_FILE_PATH = "tep.ap.completed.path";

	private static final String TEP_AP_LOCAL_ENCRYPTED_PATH = "tep.ap.encrypted.path";

	private static final String TEP_AP_SFTP_SERVER_PATH = "sftp.tep.ap.path";
	...
	public SchedulerResponseDto triggerJob(LaunchJobRequest launchJobRequest) {
		...
		String localPath = localFileBasePath + env.getProperty(TEP_AP_LOCAL_FILE_PATH);
		String sftpServerPath = env.getProperty(TEP_AP_SFTP_SERVER_PATH);
		String localEncryptedPath = localFileBasePath + env.getProperty(TEP_AP_LOCAL_ENCRYPTED_PATH);

		// upload files to sftp server
		FileMasterDao fileMaster = fileMasterRepository.findByFileGroupAndFileName(
				FileGroupEnum.ORACLE.toString(), FileMasterJobNameEnum.TEP_AP_JOB.getValue());
		...
		response.setCode(SchedulerCodeEnum.FILE_TEP_AP_JOB.toString());
		return response;
	}
