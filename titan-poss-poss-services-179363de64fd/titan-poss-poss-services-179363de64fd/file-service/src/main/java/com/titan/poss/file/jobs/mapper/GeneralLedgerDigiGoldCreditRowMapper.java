...
@Component
public class GeneralLedgerDigiGoldCreditRowMapper implements RowMapper<GeneralLedgerDto> {
	...
	public GeneralLedgerDto mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		String attribute5 = rs.getString("transaction_id"); 
		generalLedger.setAttribute5(attribute5);
		generalLedger.setAttribute6(rs.getString("doc_no"));
		generalLedger.setAttribute4("");
		generalLedger.setAttribute3(rs.getString("location_code"));
		if(rs.getString("type_of_transaction").equalsIgnoreCase("DIGI GOLD TANISHQ"))
		generalLedger.setAttribute2("DIGIGOLD BOND REDEMPTION_" + rs.getString("location_code") ); 
		else
			generalLedger.setAttribute2("DIGIGOLD BOND REDEMPTION_" + rs.getString("location_code")); 
