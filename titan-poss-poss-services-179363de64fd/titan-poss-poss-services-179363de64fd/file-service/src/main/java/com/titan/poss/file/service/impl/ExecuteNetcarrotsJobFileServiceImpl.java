...
@Slf4j
@Component
public class ExecuteNetcarrotsJobFileServiceImpl implements ExecuteJobService {

	public ExecuteNetcarrotsJobFileServiceImpl(JobFactory jobFactory) {
		jobFactory.registerJob(FileIntegrationConstants.NETCARROTS_JOB, this);
	}
	...
	private static final String NETCARROTS_LOCAL_SOURCE_PATH = "ulp.file.completed.path";

	private static final String NETCARROTS_LOCAL_ENCRYPTED_PATH = "ulp.file.encrypted.path";

	private static final String NETCARROTS_SFTP_SERVER_PATH = "sftp.netcarrots.path";
	...
	public SchedulerResponseDto triggerJob(LaunchJobRequest launchJobRequest) {
		...
		String localPath = localFileBasePath + env.getProperty(NETCARROTS_LOCAL_SOURCE_PATH);
		String localEncryptedPath = localFileBasePath + env.getProperty(NETCARROTS_LOCAL_ENCRYPTED_PATH);
		String sftpServerPath = env.getProperty(NETCARROTS_SFTP_SERVER_PATH);

		// upload files to sftp server
		FileMasterDao fileMaster = fileMasterRepository.findByFileGroupAndFileName(FileGroupEnum.NETCARROTS.toString(),
				FileMasterJobNameEnum.NC_MEMBER_DATA_JOB.getValue());
		if (fileMaster.getIsEncrypted()) {
			fileService.encryptOrdecryptFile(localPath, localEncryptedPath, "ENCRYPT");
			//log.info("File upload not done..................skipped upload and remove");
		...
		response.setCode(SchedulerCodeEnum.FILE_NETCARROTS_JOB.toString());
		return response;
	}
