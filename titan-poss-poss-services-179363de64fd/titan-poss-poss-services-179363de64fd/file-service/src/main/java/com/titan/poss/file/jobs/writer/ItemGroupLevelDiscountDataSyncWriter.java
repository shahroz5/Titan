...
@Component
public class ItemGroupLevelDiscountDataSyncWriter implements ItemWriter<DiscountItemMappingDao> {
	...
	@Override
	public void write(List<? extends DiscountItemMappingDao> items) throws Exception {
		SyncStagingDto data = getStagingDto((List<DiscountItemMappingDao>) items);
	 ...
	 * @return
	 */
	private SyncStagingDto getStagingDto(List<DiscountItemMappingDao> items) {
	...
	private List<SyncData> getSyncDataList(List<DiscountItemMappingDao> discountItemMappingDaoList) {
		List<SyncData> itemDiscountSyncDataList = new ArrayList<>();
		DiscountItemMappingSyncDto discountItemMappingSyncDto = new DiscountItemMappingSyncDto();
		if (!discountItemMappingDaoList.isEmpty())
			itemDiscountSyncDataList.add(
					DataSyncUtil.createSyncData(discountItemMappingSyncDto.getSyncDtos(discountItemMappingDaoList), 4));
