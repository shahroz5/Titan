...
@Component
public class ExecuteStockInterfaceJobServiceImpl implements ExecuteJobService {

	public ExecuteStockInterfaceJobServiceImpl(JobFactory jobFactory) {
		jobFactory.registerJob(FileIntegrationConstants.STOCK_INTERFACE_JOB, this);
	}
	...
	private static final String STOCK_INTERFACE_LOCAL_FILE_PATH = "stock.interface.completed.path";

	private static final String STOCK_INTERFACE_LOCAL_ENCRYPTED_PATH = "stock.interface.encrypted.path";

	private static final String STOCK_INTERFACE_SFTP_SERVER_PATH = "sftp.stock.interface.path";
	...
	public SchedulerResponseDto triggerJob(LaunchJobRequest launchJobRequest) {
		...
		String localPath = localFileBasePath + env.getProperty(STOCK_INTERFACE_LOCAL_FILE_PATH);
		String sftpServerPath = env.getProperty(STOCK_INTERFACE_SFTP_SERVER_PATH);
		String localEncryptedPath = localFileBasePath + env.getProperty(STOCK_INTERFACE_LOCAL_ENCRYPTED_PATH);

		// upload files to sftp server
		FileMasterDao fileMaster = fileMasterRepository.findByFileGroupAndFileName(
				FileGroupEnum.ORACLE.toString(), FileMasterJobNameEnum.STOCK_INTERFACE_JOB.getValue());
		...
		response.setCode(SchedulerCodeEnum.FILE_STOCK_INTERFACE_JOB.toString());
		return response;
	}
}
