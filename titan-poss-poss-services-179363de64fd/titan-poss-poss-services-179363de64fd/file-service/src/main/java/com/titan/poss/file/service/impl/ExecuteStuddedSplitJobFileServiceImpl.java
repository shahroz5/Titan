...
@Component
public class ExecuteStuddedSplitJobFileServiceImpl implements ExecuteJobService {

	public ExecuteStuddedSplitJobFileServiceImpl(JobFactory jobFactory) {
		jobFactory.registerJob(FileIntegrationConstants.STUDDED_SPLIT_JOB, this);
	}
	...
	private static final String STUDDED_SPLIT_JOB_LOCAL_ENCRYPTED_FILE_PATH = "studded.split.file.encrypted.path";
	...
	private static final String STUDDED_SPLIT_JOB_SFTP_RETRY_FILE_PATH = "sftp.studded.split.retry.path";
	...
	public SchedulerResponseDto triggerJob(LaunchJobRequest launchJobRequest) {
		...
		String localEncryptedPath = localFileBasePath + env.getProperty(STUDDED_SPLIT_JOB_LOCAL_ENCRYPTED_FILE_PATH);
		...
		String sftpRetryPath = env.getProperty(STUDDED_SPLIT_JOB_SFTP_RETRY_FILE_PATH);
		...
		if (files != null && files.length > 0) {
			...
			fileService.moveFilesInSftpServer(sftpSourcePath, sftpRetryPath, localRetryPath);
		...
		response.setCode(SchedulerCodeEnum.FILE_STUDDED_SPLIT_JOB.toString());
