...
@AllArgsConstructor
@NoArgsConstructor
public class GEPDigitalSignaturePrintDto implements PrintableDto {
	...
	public CustomerDocumentDto getDocumentDetails() {
		...
		customerDocument.setDocumentType(PrintDocumentTypeEnum.GEP_DIGITAL_SIGNATURE.name());
		customerDocument.setFileType(PrintFileTypeEnum.INVOICE_PRINT.name());
		customerDocument.setDocumentSubType(DocumentBucketEnum.CUSTOMER_DOCUMENT.name());
		customerDocument.setTxnId(UUID.randomUUID().toString());
	...
	public List<String> getTemplateName() {
		return List.of(FtlTemplateName.GEP_DIGITAL_SIGNATURE);
	}
}
