...
@Slf4j
@Service("salesCoinOfferDiscountServiceImpl")
public class CoinOfferDiscountServiceImpl implements DiscountService {
	...
	public CoinOfferDiscountServiceImpl(DiscountFactory discountFactory) {
		discountFactory.registerDiscountService(DiscountTypeEnum.COIN_OFFER_DISCOUNT.name(), this);
	}
	...
	public DiscountResponseDto applyTransactionLevelDiscount(SalesTxnDaoExt salesTxn,
		...
		discountBillLevelCreateDto.getDiscountDetails().forEach(discountBillDetail -> {
			...
			log.info("Sales - Coin offer eligible Item check engine API Request body - {}",
			...
			log.info("Sales - Coin offer eligible Item check engine API Response body - {}",
			...
			// Apply Filter on coin offer discount eligible item codes
			List<String> coinDiscountEligibleItemIds = eligibleDiscountItemResponse.getEligibleItemDetails().get(0)
			...
			discountUtilService.validateMaxDiscountAllowed(discountBillDetail.getDiscountValue(), maxDiscountAlloweed,
					discountBillDetail.getDiscountCode());
			...
			discountDetailsDaoExt = discountUtilService.createBillLevelDiscountDetails(discountBillDetail, salesTxn,
					...
					DiscountInitialStatusEnum.COIN_OFFER_DISCOUNT.getDiscountInitialStatus(),
			...
			discountUtilService.apportionBillLevelDiscountsToApplicableItems(discountDetailsDaoExt, salesTxn,
					coinDiscountEligibleItemIds,discountOtherDetails);
	...
	public void confirmTransactionLevelDiscount(SalesTxnDaoExt salesTxn, String discountType, String discountTxnId) {
		...
		if (!CollectionUtils.isEmpty(discountDaoList)) {
			for (DiscountDetailsDaoExt discountDetails : discountDaoList) {
				// Check at least one item should have apportioned this Coin offer discount
	...
	public void applyTransactionLevelDiscountFromOrderToCM(SalesTxnDaoExt salesTxn,
		...
		log.info("Sales - Coin offer eligible Item check engine API for Order to CM Request body - {}",
				MapperUtil.getJsonString(eligibleDiscountItemsRequestDto));
		...
		log.info("Sales - Coin offer eligible Item check engine API for Order to CM  Response body - {}",
				MapperUtil.getJsonString(eligibleDiscountItemResponse));

		// If none of the items in CM eligible for coin offer discount means ignore
		// the discount
		if (!CollectionUtils.isEmpty(eligibleDiscountItemResponse.getEligibleItemDetails())) {
			// Validate Eligible Club offer configs
			discountUtilService.validateCommonEligibleClubOfferConfigs(
			...
			log.info("Discount code :- {} and eligible item Id's - {}", orderDiscountDetailDao.getDiscountCode(),
					coinDiscountEligibleItemIds);
			...
			// Apportion the bill discounts to the eligible item Id's
			discountUtilService.apportionBillLevelDiscountsToApplicableItems(discountDetailsDaoExt, salesTxn,
					coinDiscountEligibleItemIds,null);
