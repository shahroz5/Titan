 ...
 * @version 1.0
 */
@Repository("SalesSyncStagingRepository")
public interface SalesSyncStagingRepository extends JpaRepository<SyncStaging, String>{
	
	@Query("SELECT s FROM SalesSyncStaging s WHERE s.status = 'IN_PROGRESS'")
	Page<SyncStaging> findSyncStagingDetails(Pageable pageable);
